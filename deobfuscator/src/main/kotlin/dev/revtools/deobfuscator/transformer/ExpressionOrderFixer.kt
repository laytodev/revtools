...
class ExpressionOrderFixer : Transformer {
    ...
    override fun run(group: ClassGroup) {
        group.classes.forEach { cls ->
            cls.methods.forEach { method ->
                ...
                val insns = method.instructions.toArray().filter { it.isInvertible() }

                for(insn in insns) {
                    if(insn.previous is VarInsnNode && insn.previous.previous.pushesConstant()) {
                    ...
                    else if(insn.previous.opcode == GETFIELD && (insn.previous.previous is VarInsnNode || insn.previous.previous.opcode == GETSTATIC) && insn.previous.previous.previous.pushesConstant()) {
                    ...
                    else if(insn.previous.opcode == GETSTATIC && insn.previous.previous.pushesConstant()) {
    ...
    private fun AbstractInsnNode.isInvertible(): Boolean {
        return opcode in arrayOf(IF_ICMPEQ, IF_ICMPNE, IF_ACMPEQ, IF_ACMPNE) || opcode in IMUL..DMUL || opcode in IADD..DADD
    }

    private fun AbstractInsnNode.pushesConstant(): Boolean = opcode in ICONST_M1..LDC
}