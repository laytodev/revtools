...
import dev.revtools.deobfuscator.asm.util.*
...
class BinaryExprOrderFixer : Transformer {
    ...
    override fun run(group: ClassGroup) {
        ...
        Logger.info("Reordered $count binary expressions.")
    }

    private fun AbstractInsnNode.printInsns(count: Int) {
        var cur: AbstractInsnNode? = this
        repeat(count) { cur =  cur?.previous ?: return@repeat }

        println("===================")
        repeat(count) {
            cur = cur?.next ?: return@repeat
            println(cur?.toOpcodeString())
        }
        println("===================")
    }
    ...
    private open class Expr(var type: SourceValue, vararg val values: Expr): Value {
        ...
        override fun toString(): String {
            return "(${insns.joinToString(", ") { it.toOpcodeString() }})"
        }
    }
